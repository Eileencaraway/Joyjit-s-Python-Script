...
L = size(sys.argv)
if(L<3) :
    sys.stderr.write('Usage : i. -input ii. filename ')
    print 
    exit()
...
sys.stderr.write('# For the old version of cation_interchainindex.dat use List.append(tmp) and comment out following 8 lines \n')
#################################################################################
""" reading data file """
#################################################################################
f = open(filename)
...
for j in range(0, len(data)):
    if not (data[j].startswith("#") or data[j].startswith("\n")) :
            ...
            #print len(tmp)
            #List.append(tmp)
            linej = [0 for l in range(No_peos+2)]
            linej[0] = tmp[0]
            linej[1] = tmp[1]
            l=2
            while(l<len(tmp)):
                linej[int(tmp[l])+2] = tmp[l+1]
                l += 2
...
print >> pout, '# tau3 = ', tau3woc,'+-', error3woc,'ns',  
if tdis > 0:
    print >> pout, tau3,'+-', error3,'ns',  
print >> pout  

for i in range(hist.N):
    print >> pout, (i+0.5)*hist.dx+min, " ", histwoc.rho[i],
    if tdis > 0:
        print >> pout, " ", hist.rho[i],
    print >> pout
