...
class ParticleBox:
    ...
    def __init__(self,
        ...
        self.M = M * np.ones(self.init_state.shape[0])# problem?
        self.size = size
    ...
    def init_force(self):
        ...
        #n,m=self.state.shape, it seems I don't need m
        n=len(init_state[:,0])
    ...
    def step(self, dt):
        ...
        #temporary velocity
        self.state[:,2]+=0.5*dt*self.state[:,4]/MM
        self.state[:,3]+=0.5*dt*self.state[:,5]/MM
        # update positions
        self.state[:, :2] += dt * self.state[:, 2:4]  #position also need to fix
        ...
        self.state[:,2]+=0.5*dt*self.state[:,4]/MM
        self.state[:,3]+=0.5*dt*self.state[:,5]/MM
        self.state[:,3]+=0.5*dt*(-1)*self.T/self.state[:,1]
...
dt = 1. / 1000# 30fps
# when the time is too large, particle will move too close to each other
#then the force and velcity will become unreasonable large
#thus, cause problem
#box.init_force()  # not very sure why need this here
