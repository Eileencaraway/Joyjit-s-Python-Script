...
"""
The script:
i. calculates the formula supplied as a command line argument on the specied 
   columns of the input data file. 
ii. prints out the calculated value with a reference column. Default is 
    1st Column. 
iii. calculates the cumulative sum if -csum option is supplied.    
"""
################################################################################
...
sys.stderr.write('#'+str(y_index)+'\n')
sys.stderr.write('#'+str(wy_formula)+'\n') 
...
for filename in sys.argv[carg:]:
    ...
    No_rows = len(l)
    No_columns = len(l[0])
    sys.stderr.write('# nb. of lines '+str(No_rows)+' '+str(No_columns)+'\n')
